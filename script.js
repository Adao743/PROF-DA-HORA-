...
db.collection("profissionais").orderBy("data", "desc").onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        const id = doc.id;
        lista.innerHTML += `
            <div class="bg-white rounded-xl shadow-lg mb-6 p-4 border border-gray-100">
                ...
                <div class="flex flex-col items-center mt-[-40px]">
                    ...
                    <div onclick="curtir('${id}')" class="flex items-center gap-2 mt-3 mb-3 bg-red-50 px-4 py-2 rounded-full cursor-pointer">
                        <span class="text-2xl">❤️</span>
                        <span class="font-black text-red-600">${p.likes || 0}</span>
                    </div>

                    <div class="flex gap-2 w-full">
                        <a href="https://wa.me/55${p.whatsapp}" target="_blank" class="flex-1 bg-green-500 text-white text-center py-3 rounded-xl font-bold">WHATSAPP</a>
                    </div>
                </div>
            </div>`;
    });
